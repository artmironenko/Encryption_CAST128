...
public class CAST128 {
    ...
    //функция генерации ключа (МОЖЕТ БЫТЬ ИЗМЕНЕНА НА БОЛЕЕ ЛУЧШУЮ)
    private static String keyGen(String a, String b) {
        String d = a + b; //костыль - ключ формируем из комбинации отпр-ль, пол-ль
        int d_length = d.length();
        if (d.length() < 16) {
            for (int i = 0; i < 16 - d_length; i++) {
                d += "0"; //костыль - если ключ получается коротким, ставим на конец строковые нули до ключа 128 бит
            }
        }
        return d.substring(0, 16); //substring, если ключ оказался длиннее нужной длины
    }
    ...
    //Алгоритм шифрования
    public static int[] encoding(String Text, String player1, String player2) throws UnsupportedEncodingException {
        ...
        k = getKey(key); //раскрываем ключ
        for (int i=0; i<16; i++) { //делим ключ на две части
            km[i] = k[i];
            kr[i] = k[i+16];
        }

        //Unicode плюсует в самое начало два ненужных байта
        byte[] temp = Text.getBytes("Unicode");
        byte[] temp2 = new byte[temp.length-2];
        System.arraycopy(temp, 2, temp2, 0, temp2.length); //... вырезали их

        //дополнение до 64-битовых блоков
        byte[] bytetext = new byte[temp2.length + (8 - temp2.length % 8) % 8]; //Удлиняем массив до 64*n бит
        System.arraycopy(temp2, 0, bytetext, 0, temp2.length); //Заполняем с начала (обрезать пробелы в конце будем при дешифровке)
    ...
    public static String decoding(int[] LR, String player1, String player2) throws UnsupportedEncodingException {
        //Преобразования ключа
        int[] km = new int[16];
        int[] kr = new int[16];
        int[] k; //резерв под ключ, его можно запоминать на устройстве
